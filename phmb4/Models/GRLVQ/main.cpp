...
void runExperiments(std::vector<float> params, string filePath, string testPath, string outputPath, bool normalize);
...
void classifyTestData (string testPath, string outputPath, GRLVQ &lvq, bool normalize);

int main(int argc, char** argv) {

    dbgThreshold(0);

    dbgOut(0) << "Running GRLVQ" << endl;
    ...
    for (int i = 0 ; i < inputFiles.size() - 1 ; ++i) {
        runExperiments(params, inputFiles[i], testFiles[i], resultPath, normalize);
    }
...
void runExperiments(std::vector<float> params, string filePath, string testPath, string outputPath, bool normalize) {
    ...
    for (int i=0 ; i < params.size() - 1 ; i += numberOfParameters) { // For each set of parameters
        ...
        vector<int> classes;
        map<int,int> labels;
        readFile(filePath, trainingData, classes, labels, normalize);
	...
	trainLVQ(lvq, nNodes, epochs, seed, trainingData, classes, labels);
        ...
        classifyTestData(testPath, outputPath + getFileName(testPath) + "_" + index + ".results", lvq, normalize); 
    }
}
...
void classifyTestData (string testPath, string outputPath, GRLVQ &lvq, bool normalize) {
    ...
    readFile(testPath, testData, testClasses, testLabels, normalize);
