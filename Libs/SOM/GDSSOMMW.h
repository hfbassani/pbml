...
class GDSSOMMW : public SOM<GDSNodeMW> {
public:
    uint maxNodeNumber;
    float minwd;
    float e_b;
    ...
    inline float activation(const TNode &node, const TVector &w) {
        ...
        for (uint i = 0; i < w.size(); i++) {
            ...
            //dbgOut(0) <<  node.a[i] << "\t" << w[i] << "\t" << node.w[i] << endl;
        }
...
//                node.ds[i] = epsilon_ds;
    ...
    GDSSOMMW& updateConnections(TNode *node) {
    ...
    GDSSOMMW& updateMap(const TVector &w) {
    ...
    inline GDSSOMMW& getWinners(const TVector &w, TNode* &winner1, TNode* &winner2) {
    ...
    template<class Number> GDSSOMMW& outputCentersDs(MatMatrix<Number> &m) {
        using namespace std;
