...
class GDSSOMRBF : public SOM<GDSNodeMW> {
public:
    uint maxNodeNumber;
    float minwd;
    float e_b;
    ...
    inline float activation(const TNode &node, const TVector &w) {
        ...
        return (norm / (norm + distance + 0.0000001));
    }
...
//                node.ds[i] = epsilon_ds;
    ...
    GDSSOMRBF& updateConnections(TNode *node) {
    ...
    GDSSOMRBF& updateAllConnections() {
...
//        dbgOut(1) << "winsPerNode:\t" << winsPerNode << endl;
    ...
    GDSSOMRBF& updateMap(const TVector &w) {
    ...
    inline GDSSOMRBF& getWinners(const TVector &w, TNode* &winner1, TNode* &winner2) {
    ...
    template<class Number> GDSSOMRBF& outputCentersDs(MatMatrix<Number> &m) {
        using namespace std;
